    ...
    <View>
      <View style={styling.username}>
        {fullName ? (
          ...
          <Text style={styling.editProfile} onPress={() => navigationActions.navigateProfileEdit(navigation)}>{t('Edit Profile')}</Text>
        ) : (
          <Subheading style={styling.fullName}>{path(['username'])(user)}</Subheading>
        )}
        {renderDiamond({ user, theme })}
      </View>
      {path(['bio', 'length'])(user) ? <Text>{path(['bio'])(user)}</Text> : null}
      <Caption>
